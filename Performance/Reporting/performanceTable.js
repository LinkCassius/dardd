...
class PerformanceTable extends Component {
  ...
  handleDraft = (stype) => {
    ...
    this.props.OnSaveClick(this.props.performance, "draft");
  };
  ...
  handleToggle = (stype) => {
    ...
    this.props.performance.forEach((item) => {
      ...
      if (item.checkedFlag === true && this.props.reviewType == "approvals") {
        //below conditions to check if approver has filled the remarks
        if (
          item.currentApprovar === "approverUser1" &&
          !item.approverUser1Remarks
        ) {
        ...
        //below conditions to check if approver has downloaded the docs
        if (
          item.currentApprovar === "approverUser1" &&
          !item.apUser1HasDownload
        ) {
          countApprovalHasDocs++;
        } else if (
          item.currentApprovar === "approverUser2" &&
          !item.apUser2HasDownload
        ) {
          countApprovalHasDocs++;
        } else if (
          item.currentApprovar === "approverUser3" &&
          !item.apUser3HasDownload
        ) {
          countApprovalHasDocs++;
        }
      }
    ...
    if (countApprovalHasDocs > 0) {
      NotificationManager.error(
        "Please download & view all PoEs for the selected indicator(s)"
      );
      return;
    }
    if (stype === "submit") {
      submitType = "submit";
      message = "Do you want to submit the indicator(s) for approval";
    } else if (stype === "approved") {
      submitType = "approved";
      message = "Do you want to approve the indicator(s)";
    } else if (stype === "rejected") {
      submitType = "rejected";
      message = "Do you want to reject the indicator(s)";
    }
    this.setState({
  ...
  render() {
    ...
    //console.log("performacnce : ", performance);
    // let docFlag = false;
    ...
    if (this.props.reviewType === "self") {
      ...
      this.columns[5].name = "Challenges/Comment";
      this.columns[6].options.display = true; //intervention
      this.columns[7].options.display = false; //responsibility
    } else {
      approvalsStatusFlag = performance.find(
        (e) => e.currentApprovarStatus === "pending"
      );
      this.columns[5].name = "Remarks";
      this.columns[6].options.display = false;
      this.columns[7].options.display = true; //responsibility
    }
    ...
    return (
      <div>
        ...
        <div className="text-right">
          {this.props.reviewType === "self" ? (
            <div>
              {performance && typeof selfStatusFlag === "undefined" ? (
                <span>&nbsp;&nbsp;</span>
              ) : (
                <div>
                  {" "}
                  ...
                  <Button
                    ...
                    color="warning"
                    onClick={() => this.handleToggle("submit")}
                  >
                    Submit
                  </Button>
                </div>
            ...
            <div>
              {
                // (performance && (performance[0].currentApprovarStatus != "pending")) ?

                performance && typeof approvalsStatusFlag === "undefined" ? (
                  <span>&nbsp;&nbsp;</span>
                ) : (
                  <div>
                    <Button
                      name="isRejected"
                      color="danger"
                      onClick={() => this.handleToggle("rejected")}
                    >
                      Reject
                    </Button>
                    <span>&nbsp;&nbsp;</span>
                    <Button
                      color="primary"
                      onClick={() => this.handleToggle("approved")}
                    >
                      Approve
                    </Button>
                  </div>
