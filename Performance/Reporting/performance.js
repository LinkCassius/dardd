...
import PerformanceDocs from "../Documents/performance.documents";
...
class Performance extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      //below var are for passing to child component (docs) to update in db to make 'save as draft' optional
      actualP: "",
      challengesP: "",
      interventionP: "",
    };
  ...
  OnSaveClick = (Plist, status) => {
    ...
    var validationApprovalRemarksflag = false;
    var validationSelfRemarksflag = false;
    var withOutDataflag = false;

    
    for (var item in Plist) {
      if (
        Plist[item].checkedFlag === false &&
        status !== "submit" &&
        (Plist[item].actualPerformance !== "" ||
          Plist[item].intervention !== "" ||
          Plist[item].remarks !== "")
      ) {
        console.log("plist without data flag : ", Plist[item]);
        withOutDataflag = true;
        break;
      }

      if (
        Plist[item].actualPerformance === "" &&
        Plist[item].checkedFlag === true
      ) {
        validationflag = true;
        break;
      }
      ...
      if (
        status === "submit" &&
        Plist[item].checkedFlag === true &&
        (Plist[item].intervention === "" || Plist[item].remarks === "")
      ) {
        validationSelfRemarksflag = true;
      }
      if (
        Plist[item].checkedFlag == true &&
        ...
        if (
          Plist[item].currentApprovar === "approverUser1" &&
          !Plist[item].approverUser1Remarks
        ) {
          validationApprovalRemarksflag = true;
          break;
        } else if (
          Plist[item].currentApprovar === "approverUser2" &&
          !Plist[item].approverUser2Remarks
        ) {
          validationApprovalRemarksflag = true;
          break;
        } else if (
          Plist[item].currentApprovar === "approverUser3" &&
          !Plist[item].approverUser3Remarks
        ) {
          validationApprovalRemarksflag = true;
          break;
        }
    ...
    if (validationSelfRemarksflag) {
      NotificationManager.error("Please fill challenges & intervention fields");
      return;
    }
    if (validationApprovalRemarksflag) {
      NotificationManager.error("Please fill remarks");
      return;
    }
    if (withOutDataflag) {
      NotificationManager.error("Please select the record with entered data");
      return;
    }
    */
    let PlistFilter = Plist.filter((el) => el.checkedFlag === true);
    console.log("PlistFilter : ", PlistFilter);
        ...
        if (data && data.success === true) {
          const notiMessage =
            status === "draft"
              ...
              : status === "submit"
              ? "Performance reporting sent for approval"
              : status === "rejected"
              ? "Performance reporting rejected successfully"
              : status === "approved"
              ? "Performance reporting approved successfully"
              : "";
  ...
  //update indicator values (actual, challenges, intervention) on clicking doc folder
  updateIndicatorOnFolder = (pid, perfList) => {
    let PlistFilter = perfList.filter((el) => el._id === pid);

    fetch(ApiEndPoints.updateIndicatorByFolder, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
                    ...
                    <GridItem xs={12} sm={12} md={12}>
                      {perList.map((list) => {
                        ...
                        return PsList1.map((ilist, index) => {
                          if (ilist.length > 0) {
                            let quarter = childvalue[index].includes("Apr")
                              ? "Q1"
                              : childvalue[index].includes("Jul")
                              ? "Q2"
                              : childvalue[index].includes("Oct")
                              ? "Q3"
                              : childvalue[index].includes("Jan")
                              ? "Q4"
                              : "";
                            return (
                              <PerformanceTable
                                ...
                                onFileOpen={(id, perfList) => {
                                  ...
                                  this.updateIndicatorOnFolder(id, perfList);
                                }}
                      ...
                      {indicator && (
                        <TableContainer>
                          <Table responsive="true">
                            ...
                            <TableBody>
                              <TableRow
                                key={indicator.responsibleUser.firstName}
                              >
                              ...
                              <TableRow key={indicator.approverUser1.firstName}>
                                {/* <TableCell component="th" scope="row">
                                  Approvar
                              </TableCell> */}
                                <TableCell>
